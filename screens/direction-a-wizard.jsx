// Env wizard — interactive .env builder.
// Mounts as a tab inside ConsoleA. No external deps.

function EnvWizard() {
  const { useState: useS, useMemo: useM } = React;

  const [cfg, setCfg] = useS({
    GITHUB_TOKEN: "ghp_••••••••••••••••••••••",
  ...
  // Derived state
  const mode = cfg.GITHUB_READ_ONLY
    ? "read_only"
    : !cfg.GITHUB_ENABLE_DANGEROUS_TOOLS
    ? "write_safe"
    : "operator";
  ...
  const envOutput = useM(() => {
    const lines = [
      ...
      cfg.GITHUB_DESTRUCTIVE_CONFIRMATION
        ? "GITHUB_DESTRUCTIVE_CONFIRMATION=CONFIRM_DESTRUCTIVE_OPERATION"
        : "GITHUB_DESTRUCTIVE_CONFIRMATION=",
  ...
  const copy = () => navigator.clipboard?.writeText(envOutput);
  const [copied, setCopied] = React.useState(false);
  const handleCopy = () => { copy(); setCopied(true); setTimeout(() => setCopied(false), 1800); };
      ...
      <div className="ca-wizard-layout">
        ...
        <div className="ca-wizard-controls">
          ...
          <WizSection title="02 · Postura de escrita">
            <WizToggle
              label="GITHUB_READ_ONLY"
              value={cfg.GITHUB_READ_ONLY}
              onChange={v => { set("GITHUB_READ_ONLY", v); if (v) { set("GITHUB_ENABLE_DANGEROUS_TOOLS", false); set("GITHUB_DESTRUCTIVE_CONFIRMATION", false); } }}
              hint="true = nenhuma mutação possível. Recomendado para primeiro deploy."
              tone={cfg.GITHUB_READ_ONLY ? "ok" : "warn"}
            />
            ...
            <WizToggle
              label="GITHUB_ENABLE_DANGEROUS_TOOLS"
              value={cfg.GITHUB_ENABLE_DANGEROUS_TOOLS}
              ...
              tone={cfg.GITHUB_ENABLE_DANGEROUS_TOOLS ? "warn" : "ok"}
              disabled={cfg.GITHUB_READ_ONLY}
            />
            <WizToggle
              label="GITHUB_ALLOW_WORKFLOW_DISPATCH"
              value={cfg.GITHUB_ALLOW_WORKFLOW_DISPATCH}
              onChange={v => set("GITHUB_ALLOW_WORKFLOW_DISPATCH", v)}
              hint="Habilita actions_run_workflow. Manter false como padrão."
              tone={cfg.GITHUB_ALLOW_WORKFLOW_DISPATCH ? "warn" : "ok"}
            />
            <WizToggle
              label="GITHUB_DESTRUCTIVE_CONFIRMATION"
              value={cfg.GITHUB_DESTRUCTIVE_CONFIRMATION}
          ...
          <WizSection title="03 · FastMCP — rede">
            <WizRow2>
              <WizField label="FASTMCP_HOST" hint="">
                <input className="wiz-input mono" value={cfg.FASTMCP_HOST} onChange={e => set("FASTMCP_HOST", e.target.value)} />
              </WizField>
              <WizField label="FASTMCP_PORT" hint="">
                <input className="wiz-input mono" value={cfg.FASTMCP_PORT} onChange={e => set("FASTMCP_PORT", e.target.value)} style={{ width: 90 }} />
              </WizField>
            </WizRow2>
            <WizToggle
              label="FASTMCP_ALLOW_REMOTE_HOSTS"
              value={cfg.FASTMCP_ALLOW_REMOTE_HOSTS}
              onChange={v => set("FASTMCP_ALLOW_REMOTE_HOSTS", v)}
              hint="Use só para validação por túnel. Para deploy fixo, use ALLOWED_HOSTS."
              tone={cfg.FASTMCP_ALLOW_REMOTE_HOSTS ? "warn" : "ok"}
            />
          ...
          <WizSection title="04 · OAuth & rate limit">
            ...
            <WizToggle label="MCP_OAUTH_RATE_LIMIT_ENABLED" value={cfg.MCP_OAUTH_RATE_LIMIT_ENABLED} onChange={v => set("MCP_OAUTH_RATE_LIMIT_ENABLED", v)} hint="Desabilitar apenas em dev local." tone={cfg.MCP_OAUTH_RATE_LIMIT_ENABLED ? "ok" : "warn"} />
            <WizRow2>
              <WizField label="WINDOW_SECONDS" hint="">
                <input className="wiz-input mono" value={cfg.MCP_OAUTH_RATE_LIMIT_WINDOW_SECONDS} onChange={e => set("MCP_OAUTH_RATE_LIMIT_WINDOW_SECONDS", e.target.value)} style={{ width: 80 }} />
              </WizField>
              <WizField label="MAX_REQUESTS" hint="">
                <input className="wiz-input mono" value={cfg.MCP_OAUTH_RATE_LIMIT_MAX_REQUESTS} onChange={e => set("MCP_OAUTH_RATE_LIMIT_MAX_REQUESTS", e.target.value)} style={{ width: 80 }} />
              </WizField>
              <WizField label="TOKEN_TTL_SECONDS" hint="">
                <input className="wiz-input mono" value={cfg.MCP_OAUTH_ACCESS_TOKEN_TTL_SECONDS} onChange={e => set("MCP_OAUTH_ACCESS_TOKEN_TTL_SECONDS", e.target.value)} style={{ width: 100 }} />
              </WizField>
            </WizRow2>
          </WizSection>
        </div>
        ...
        <div className="ca-wizard-output">
          <div className="ca-wiz-out-h">
            ...
            <button className="ca-wiz-copy" onClick={handleCopy}>
              {copied ? "✓ copiado" : "copy .env"}
            </button>
            <button className="ca-wiz-copy ca-wiz-download" onClick={handleDownload}>
              ↓ .env
            </button>
          </div>
...
function WizToggle({ label, value, onChange, hint, tone = "ok", disabled = false }) {
  const toneColor = { ok: "var(--ok)", warn: "var(--warn)", danger: "var(--danger)" }[tone];
  return (
