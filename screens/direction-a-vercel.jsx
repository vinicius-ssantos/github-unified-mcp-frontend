// Direction A — Vercel Preview Deploy screen
// Fluxo: validate credentials → plan (dry-run) → deploy preview → poll status
// Demo mode com dados mock realistas quando serverUrl não configurado.

(function () {
  const { useState, useEffect, useCallback, useRef } = React;

  // ── Demo data ─────────────────────────────────────────────────────────────
  const DEMO_CREDS = { valid: true, user: "vinicius-ssantos", team: null };
  ...
  const DEMO_PLAN = {
    ...
    public_env_check: {
      found: ["VITE_MCP_URL"],
      warnings: ["VITE_MCP_URL é exposto client-side — certifique-se de que é um endpoint público"],
    },
  };
  ...
  const DEMO_STATUS_SEQ = ["QUEUED", "BUILDING", "BUILDING", "READY"];
  ...
  function VBadge({ status }) {
    ...
    }[status] || { bg: "rgba(180,180,200,.1)", color: "#8392b5", border: "rgba(145,166,216,.2)" };
    return (
      <span style={{ ...cfg, display: "inline-block", borderRadius: 999, padding: "3px 11px", fontSize: 11, textTransform: "uppercase", letterSpacing: ".08em", fontWeight: 700, fontFamily: "ui-monospace, monospace", border: `1px solid ${cfg.border}` }}>
        {status}
      </span>
    );
  }
  ...
  function VField({ label, hint, children }) {
    return (
      <div style={{ display: "grid", gap: 5, fontSize: 13, color: "var(--text-dim,#888)" }}>
        ...
        {children}
        {hint && <p style={{ margin: 0, fontSize: 11, color: "var(--text-muted,#555)", lineHeight: 1.4 }}>{hint}</p>}
      </div>
    );
  }

  function VPanel({ children, style }) {
    return (
      <div style={{
        background: "var(--surface-2,#0d131f)",
        border: "1px solid var(--border,#1c2538)",
  ...
  // ── Main component ────────────────────────────────────────────────────────
  function VercelDeployA({ serverUrl, bearerToken = "" }) {
    ...
    const handleValidate = async () => {
      ...
      try { setCreds(await callTool("vercel_validate_credentials", {})); }
    ...
    const handlePlan = async () => {
      ...
      try { setPlan(await callTool("vercel_project_plan", { ...form })); }
      catch (e) { setError(e.message); }
      setStep("idle");
    };
    ...
    const handleDeploy = async () => {
      ...
      if (isDemo) {
        ...
        pollRef.current = setInterval(() => {
          ...
          if (idx >= DEMO_STATUS_SEQ.length - 1) {
            ...
            setDeployStatus(s => ({ ...s, status: "READY", preview_url: DEMO_PREVIEW_URL }));
            setStep("idle");
          } else {
            demoSeqRef.current = idx + 1;
            setDeployStatus(s => ({ ...s, status: DEMO_STATUS_SEQ[demoSeqRef.current] }));
          }
        }, 2000);
        return;
      }
      ...
      try {
        const result = await callTool("vercel_deploy_preview", { ...form, approval: "APPROVED", ci_gate: true });
        setDeploymentId(result.deployment_id);
        setDeployStatus({ id: result.deployment_id, status: result.status, url: result.url });
        setStep("polling");
        let pollErrors = 0;
        pollRef.current = setInterval(async () => {
          try {
            const st = await callTool("vercel_get_deploy_status", { deployment_id: result.deployment_id });
            pollErrors = 0; setDeployStatus(st);
            if (["READY", "ERROR", "CANCELED"].includes(st.status)) { stopPolling(); setStep("idle"); }
          } catch (e) {
            if (++pollErrors >= 3) { stopPolling(); setStep("idle"); setError(`Polling interrompido: ${e.message}`); }
          }
        }, 5000);
    ...
    const handleRefreshStatus = async () => {
      ...
      try { setDeployStatus(await callTool("vercel_get_deploy_status", { deployment_id: deploymentId })); }
      catch (e) { setError(e.message); }
    };
    ...
    const btnSt = (disabled, variant = "default") => ({
          ...
          {plan && (
            <div style={{ borderTop: "1px solid var(--border,#1c2538)", paddingTop: 14, display: "flex", flexDirection: "column", gap: 11 }}>
              <div style={{ display: "grid", gap: 6 }}>
                {[["project_name","Project"],["github_repo","Repo"],["branch","Branch"],["framework","Framework"],["build_command","Build"],["output_dir","Output dir"]].map(([k, lbl]) => (
                  <div key={k} style={{ display: "grid", gridTemplateColumns: "110px 1fr", gap: 10, fontSize: 12 }}>
                    <span style={{ color: "var(--text-dim,#888)" }}>{lbl}</span>
                    <span style={{ fontFamily: "ui-monospace, monospace", color: "var(--text,#c8d8f8)" }}>{plan[k] || "—"}</span>
                  </div>
                ))}
              </div>
  ...
  window.VercelDeployA = VercelDeployA;
})();
